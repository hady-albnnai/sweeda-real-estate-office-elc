-- 2026-07-14 — تفعيل النشر التلقائي افتراضياً بعد الموافقة
-- المطلوب حسب طلب المالك: النشر يحدث تلقائياً فور موافقة الإدارة بدون زر يدوي إضافي.

BEGIN;

-- 1) تأكيد وجود جدول social_publications وفهرسه (إعادة آمنة إذا الميجريشن السابق لم يُطبق)
...
-- 2) دالة claim اضمنها موجودة (نسخة من migration السابق للتوافق)
...
-- 4) تفعيل النشر التلقائي افتراضياً — منذ الآن true وليس false
UPDATE public.app_config
SET value = jsonb_set(
  value,
  '{socialPublishing}',
  COALESCE(value->'socialPublishing', '{}'::jsonb) || '{"autoPublish": true}'::jsonb,
  true
)
WHERE key = 'main';

-- 5) تأكيد أن الموجود مسبقاً لو كان false يصبح true (حسب طلب المالك الأخير)
UPDATE public.app_config
SET value = jsonb_set(value, '{socialPublishing,autoPublish}', 'true'::jsonb, true)
WHERE key = 'main'
  AND (value->'socialPublishing'->>'autoPublish') IS DISTINCT FROM 'true';
