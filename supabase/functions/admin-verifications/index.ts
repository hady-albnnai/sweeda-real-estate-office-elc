// Edge Function: admin-verifications
// الغرض: نقل مراجعة توثيق المستخدمين من RPC/قراءة مباشرة إلى Edge Function محمية بجلسة موظف.
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "approve") {
      const targetUid = (body.target_uid ?? body.targetUid ?? body.user_id ?? body.userId)?.toString() ?? "";
      if (!targetUid) return json({ success: false, error: "TARGET_UID_REQUIRED" }, 400);
      const { data, error } = await supabaseAdmin.rpc("admin_approve_verification_by_admin", {
    ...
    if (action === "reject") {
      ...
      if (!targetUid) return json({ success: false, error: "TARGET_UID_REQUIRED" }, 400);
      const { data, error } = await supabaseAdmin.rpc("admin_reject_verification_by_admin", {
        p_admin_uid: adminUid,
        p_target_uid: targetUid,
        p_reason: reason,
      });
