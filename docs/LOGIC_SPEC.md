...
- **الروابط القابلة للإدارة** (في `app_config.txts`):
  - `facebook` — رابط صفحة فيسبوك الرسمية
  - `instagram` — رابط حساب إنستغرام الرسمي
  - `developerPhone` — رقم هاتف المطور (يظهر في "عن التطبيق")
  - `socialPages` — خريطة إضافية (مثال: `{"tiktok": "https://..."}`) للتوسعة المستقبلية (تيك توك، إلخ).
- **تظهر الروابط** تلقائياً في حوار "عن التطبيق" (settings_screen) + يمكن إضافتها من `/admin/config`.
- **النشر التلقائي**:
  - في شاشة **إضافة عرض** (`add_offer_screen`): تشيك بوكس مفعّل تلقائياً "نشر العرض تلقائياً على صفحاتنا...".
  - عند إضافة العرض:
    - `i_soc = 1` إذا كان التشيك بوكس مفعّل (الافتراضي).
    - `socTxt` = نص منشور جاهز يُولّد تلقائياً من بيانات العرض (عنوان + نوع + سعر + منطقة + وصف + هاشتاجات).
  - بعد موافقة الإدارة على العرض (`sts=2`): إذا `i_soc=1` → يُستخدم `socTxt` للنشر اليدوي/الآلي (في المستقبل عبر Edge Function أو Meta API).
  - يمكن للمستخدم إلغاء التفعيل قبل الإرسال.
- **القالب** (مولّد داخل التطبيق):
  ```
  ...
  📌 المنطقة: ...
  [الوصف]
  للتفاصيل والحجز: تطبيق عقارات السويداء
  ```
- **المحافظة على الميزات السابقة**: لا تتأثر ميزة طلبات الفيديو أو التحقق من الهاتف.
