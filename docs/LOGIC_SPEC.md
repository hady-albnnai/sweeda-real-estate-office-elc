...
- **مسار طلب الفيديو** (من offer_detail_screen): يستخدم نفس الـ Sheet مع علم `isVideoRequest=true`.
  - بعد نجاح الحجز مباشرة: يتم فتح واتساب تلقائياً (url_launcher) على الرقم المخصص (`app_config.texts.videoRequestWhatsApp` أو fallback) مع رسالة جاهزة تحتوي `#${offerNumber}`.
