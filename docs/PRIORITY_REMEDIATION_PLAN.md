...
| P1 | أمان الإدارة والمصادقة | إلغاء الثقة بـ `admin_uid` وحده | الأهم |
...
- [ ] إنشاء migration لجداول ودوال الجلسات.
- [ ] تعديل `login_with_password` أو إضافة RPC مساعدة لإصدار جلسة.
- [ ] تعديل `AuthProvider.loginWithPassword` لتخزين التوكن.
- [ ] إضافة دالة في `AuthProvider` أو `AuthService` لإرجاع توكن الإدارة الحالي.
- [ ] تعديل `AdminProvider` لإرسال `staff_session_token` مع كل Edge Function حساسة.
- [ ] تعديل Edge Functions للتحقق من الجلسة قبل تنفيذ RPC الحساسة.
- [ ] إضافة revoke عند تسجيل الخروج.
- [ ] إضافة انتهاء صلاحية للجلسة.
- [ ] اختبار أن من يعرف `admin_uid` فقط لا يستطيع تنفيذ أي عملية.
...
- [ ] استخراج Staff Admin إلى `StaffAdminService`.
