...
- `user_details_screen`: عرض حالة الباقة للإدارة (نشطة / فترة سماح X يوم / مجانية)
...
- المشكلة: عند انقطاع السلسلة (غياب أكثر من يوم) كان streak يزيد بدل أن يُصفَّر
- الإصلاح: إضافة `v_yesterday` + منطق ثلاثي:
  - `v_last_day IS NULL` → `1` (مستخدم جديد)
  - `v_last_day = v_yesterday` → `strk + 1` (يكمل)
  - غير ذلك → `1` (يُصفَّر)

**الكلاينت (`user_home_screen`):**
- حذف guard مكرر لفحص اليوم — الـ guard الكامل موجود في `auth_provider.registerStreak`
