...
│   ├── ph ← الهاتف
│   ├── ad ← العنوان
│   ├── role ← المستوى: 0=مستخدم, 1=وسيط, 2=مصور, 3=مشرف, 4=موظف مكتب, 5=نائب مدير, 6=مدير
│   ├── sid ← رقم الهوية
...
│   ├── perm ← صلاحيات داخلية مخصصة (JSONB array، فارغة = صلاحيات الدور الافتراضية)
...
│   ├── cat ← النوع الرئيسي (id رقمي من مفاتيح catProp/catVeh في config، مثل 0=سكني)
│   ├── sub ← النوع الفرعي (index رقمي 0-based داخل مصفوفة "sub" التابعة للمجموعة الرئيسية في config)
│   ├── ttl ← العنوان
│   ├── prc ← السعر
│   ├── cur ← العملة: 0=$, 1=ل.س
...
```javascript
{
  // —— نقاط الاكتساب ——
  "pts": {
    "sgn": 1000, "wkL": 100, "addO": 500, "att": 300,
  ...
  "clrs": [...],
  // —— صلاحيات الأدوار ——
  "roles": {"0":{"nm":"مستخدم"},"1":{"nm":"وسيط"},"2":{"nm":"مصور"},"3":{"nm":"مشرف"},"4":{"nm":"موظف مكتب"},"5":{"nm":"نائب مدير"},"6":{"nm":"مدير"}}
}
```
...
## تحديث 2026-06-12 — إصلاح حجز المواعيد ودورة حياة العرض

### `book_appointment_internal` — أُعيد كتابتها بالكامل

الفحوصات بالترتيب:
1. العرض موجود ومنشور (`sts=2`)
2. ما يحجز عرضه الشخصي
3. الوقت بالمستقبل
4. ما في طلب إتمام معلق على العرض (`completion_requests.decision='pending'`)
5. فحص `avl` (يوم + فترة زمنية بتوقيت `Asia/Damascus`)
6. فحص تعارض العقار (ما في موعد `sts IN (0,1)` بنفس الوقت)
7. منع تكرار نفس المستخدم على نفس العرض
8. اختيار منفذ (`role=3`) متاح — الأقل مواعيداً ← الأقدم تسجيلاً
9. إنشاء مع `supervisor_uid` + `sts=0`
10. إرجاع `JSONB` يتضمن `active_appointments` (عدد المواعيد النشطة)

### دورة حياة العرض — تحديث تلقائي

| الحدث | `offers.sts` | `i_pub` | المواعيد الأخرى |
|---|---|---|---|
| إنشاء عرض | 1 (مراجعة) | 0 | — |
| قبول المراجعة | 2 (منشور) | 1 | — |
| رفض المراجعة | 3 (مرفوض) | 0 | — |
| انتهاء (30 يوم) | 4 (منتهي) | 0 | — |
| تجديد بالنقاط (ren) | 2 (منشور) | 1 | — |
| موافقة على طلب إتمام | 5 (محجوز) | 0 | تُلغى + إشعار |
| إنشاء صفقة | 5 (محجوز) | 0 | — |
| إكمال صفقة | 6 (مكتمل) | 0 | تُلغى |

### حدود الأدوار النهائية في RPCs

| الحدّ | الدوال |
|---|---|
| `role >= 4` (موظف فما فوق) | admin_force_appointment, admin_handle_report, process_completion_request, create_request إعفاء, create_offer إعفاء |
| `role >= 5` (نائب/مدير) | create_deal, complete_deal, approve_payment, manage_deals, manage_payments |
| `role >= 6` (مدير فقط) | app_config كتابة |


## تحديث 2026-06-12 — نظام المنفذ الميداني

تمت إضافة تدفق كامل للمنفذ الميداني (المشرف) مع المكتب:

### جدول جديد: `completion_requests`
يخزن طلبات إتمام المعاملات — المنفذ يطلب، المكتب (مدير/نائب/موظف) يراجع.

| العمود | النوع | الغرض |
|---|---|---|
| `id` | UUID PK | |
| `app_id` | UUID FK→appointments | الموعد المرتبط |
| `req_by` | UUID FK→users | المنفذ الذي طلب الإتمام |
| `notes` | TEXT | ملاحظات المنفذ |
| `decision` | TEXT | pending / approved / rejected |
| `decided_by` | UUID FK→users | من اتخذ القرار |
| `office_notes` | TEXT | ملاحظات المكتب |
| `ts_crt` | TIMESTAMPTZ | تاريخ الطلب |
| `ts_decided` | TIMESTAMPTZ | تاريخ القرار |

### أعمدة جديدة في `appointments`
| العمود | النوع | الغرض |
|---|---|---|
| `outcome` | TEXT | نتيجة التنفيذ: accept/reject/postpone |
| `executor_notes` | TEXT | ملاحظات المنفذ |
| `rejection_reason` | TEXT | سبب الرفض |
| `completion_date` | TIMESTAMPTZ | تاريخ التنفيذ الفعلي |

### دوال RPC الجديدة
| الدالة | الغرض |
|---|---|
| `get_my_tasks` | مهام المنفذ اليوم |
| `get_postponed_tasks` | المهام المؤجلة |
| `get_completed_tasks` | المهام المنفذة |
| `update_task_outcome` | تأجيل/رفض مهمة |
| `request_completion_by_appointment` | طلب إتمام معاملة |
| `get_all_pending_completion_requests` | طلبات معلقة (للمكتب) |
| `process_completion_request` | موافقة/رفض طلب إتمام |

### المسارات
| المسار | الشاشة |
|---|---|
| `/executor/tasks` | مهام المنفذ (4 تبويبات) |
| `/executor/execute/:id` | تنفيذ مهمة |


## تحديث 2026-06-10 — مهام التصوير

تمت إضافة جدول `photography_tasks` لإدارة تكليف المصورين بالعروض ورفع الوسائط ومراجعة المكتب لها. الحالات: 0 بانتظار المصور، 1 قيد التنفيذ، 2 مرسلة للمكتب، 3 معتمدة، 4 مرفوضة، 5 ملغاة.


## تحديث 2026-06-10 — إدارة الأدوار ومنع تكرار الهاتف

تمت إضافة دوال `admin_update_user_role` و `admin_set_user_status` لتحديث أدوار وحالات المستخدمين عبر RPC متوافقة مع وضع المصادقة الحالي. كما تمت إضافة `normalize_sy_phone` وفهرس فريد على الرقم الموحّد لمنع إنشاء أكثر من حساب لنفس رقم الهاتف بصيغ مختلفة.
