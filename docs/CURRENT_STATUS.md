...
**آخر تحديث:** 2026-07-30 — ✅ إقفال مسار التصوير بالكامل + إصلاح 22 مسار إشعار بلا بوش
**الفرع:** `main` (commit `4656e13`)
**الحالة العامة:** ✅ جاهز للإطلاق — 178+ دالة محصّنة، 33 Edge Function منشورة.

---

### 📸 جولة 2026-07-30 — إقفال التصوير + الإشعارات

| البند | الحالة |
|---|---|
| **مسار التصوير** | ✅ **مُقفل بالكامل** — 14 فجوة (تفاصيلها `docs/2026_07_29_SESSION_SUMMARY.md`) |
| منع الموعد الماضي · إعادة الإسناد · اعتذار المصوّر | ✅ `769edd1` |
| منع فقدان الصور · منع تعارض مواعيد المصوّر | ✅ `e941fcb` |
| **«عرضك» → «العرض الخاص بك»** | ✅ `4656e13` — 11 ملفاً + الدوال الحية (§19 بالدستور) |
| **🔴 22 من 28 مسار إشعار بلا بوش** | ✅ الجذر `notify_user()` أُصلح بالمنبع — نشر/رفض العرض · رد المعاينة · انتهاء الاشتراك · المحامي/المعقّب كلها صارت تصل الجهاز |
| إحصاءات التصوير للمكتب | ✅ `2270036` |
| أجر التصوير (1000 ل.س قابل للتعديل) | ✅ `1c0c1f0` |

**الدستور:** أُضيفت §16 (منع التعليق) · §17 (Management API) · §18 (دروس فنية مكلفة) · §19 (لغة الواجهة).

---

### 🔴 جولة 2026-07-29 — الفجوات الثلاث المعلّقة أُقفلت (`f7c95c1`)

| # | الفجوة | الجذر | الحالة |
|---|--------|-------|--------|
| ① | **الإعلان المميّز المدفوع لا يظهر مميّزاً** | `approve_payment_final` تكتب `fms_end` دون `i_fms` بينما الشارة/الترتيب/الكنس مشروطة به ⇒ الزبون يدفع ولا يحصل على شيء | ✅ الدالة + الكنس + تصحيح بيانات · «يتيني» `i_fms 0→1` · `orphans=0` |
| ② | **وثيقة الملكية مكشوفة للعامة** | `doc_img` رابط `public` يُجلب بلا مفتاح (200 · 289KB) والحماية بالواجهة تجميلية | ✅ جدول `offer_documents` محمي بـRLS + تريغر · الجدول 401 للزائر |
| ③ | **زر «حفظ الإعدادات» معطّل خادمياً** | `updateConfig` يكتب `app_config` بـPATCH بمفتاح anon ⇒ `42501` ⇒ لا شيء يُحفظ رغم رسالة النجاح | ✅ أكشن `update_sections` بمفتاح الخدمة + قائمة بيضاء + دمج بلا فقدان |

**⚠️ درس مسجَّل (②):** محاولة `REVOKE SELECT` + منح عمودي **كسرت التطبيق** — العميل يستخدم `.select()` المفتوحة في 12+ موضع وPostgres يرفضها `42501` عند نقص أي عمود. تراجُع فوري وإثبات رجوع 200. **منع الأعمدة غير متوافق مع `select(*)`.**

📌 اكتُشف أن `socialPublishing` كان **مفقوداً من `app_config` الحي** (سابق للتعديل) — أُضيف.

---

### 📸 جولة 2026-07-28/29 — مسار التصوير (`33e5b02` + `1c0c1f0`)

- **6 فجوات مُغلقة:** الساعة بالعرض · إشعار المصوّر الغني · إشعار صاحب الطلب عند الإسناد · **البوش الخارجي** (كان معدوماً — لا استدعاء لـ`send_push_notification` إطلاقاً) · الدمج بـ«مواعيدي» · إشعارا البدء والتسليم.
- **تذكير قبل ساعة** للطرفين: `send_photography_reminders` (رباعي تحصين · dedup `rmnd_1h` · كرون `*/15`).
- **أجر التصوير** 1000 ل.س قابل للتعديل من الإدارة (`photoPrice`، و0 = مجاني فيُخفى).
- **E2E كامل مُثبت:** 5 حلقات (`0→1→2→3`) + 4 إشعارات بوش وصلت جهاز المالك.

---

### 🔧 إصلاح البناء (2026-07-28)
**الجذر:** فشل DNS لـ`storage.googleapis.com` — **لا علاقة له بالكود ولا بإصدار Flutter** (`flutter analyze` على 3.44.8 = 0 أخطاء).
**الحل:** `FLUTTER_STORAGE_BASE_URL` + `PUB_HOSTED_URL` لمرآة بديلة. نجح ✅

---

### 🔒 السحب الأمني الشامل (2026-07-28) — 176 دالة
**157** محصّنة `42501` · **18** تريغر `PGRST202` (سليم بالتصميم) · **1** overload · **صفر تسريب**.
كل الإيدج ترفض بلا جلسة · RLS: anon يقرأ `offers`+`app_config` فقط · `app_config` بلا أسرار.

**تصحيح لسجل 2026-07-27:** `ratings` **يرفض الإدراج بـ42501** على مستوى الصلاحية — أقوى مما وُثّق سابقاً («التريغر يحرسه»).

**التفاصيل الكاملة:** `docs/2026_07_29_SESSION_SUMMARY.md`

---

### 🌙 جولة 2026-07-28 — إقفال فجوتي إشعارات المواعيد + إصلاح تظهير الدفعات

### ✅ المُنجز
| البند | التفاصيل |
|-------|----------|
| **ربط Supabase CLI** | ✅ باستخدام PAT Token عبر `supabase login --token` و`supabase link` |
| **(أ) إشعار القبول الغني** | تريغر `trg_appt_accept_enrich` — يكتب عنوان العرض + التاريخ + الساعة + اسم المشرف وهاتفه + «تواصل قبل ساعة». مُثبت حيّاً: *«يرجى الحضور إلى موقع العرض المطلوب والمحدد بـ: طرطوس وذلك بتاريخ 2026/07/29 الساعة 11:00. يرجى التواصل على رقم المشرف مشرف تجريبي ١ — +963957010003 قبل ساعة كاملة من الموعد.»* |
| **(ج) إشعار المشرف المُسنَد** | تريغر `trg_appt_notify_supervisor` — لحظة الحجز ← المشرف يستلم `📋 أُسند إليك موعد معاينة` مع تفاصيل العرض والتاريخ والموقع. مُثبت حيّاً |
| **(ب) التذكيرات الفعلية** | 3 عتبات (24س/2س/15د) لكل من الطالب والمشرف بنص مخصّص. دالة `send_appointment_reminders` أُعيد بناؤها بالكامل (CTE UPDATE + `_appt_reminder_fanout`). كرون جديد `appointment-reminders-15min` بدقة `*/15 * * * *` |
| **نشر `user-account`** | مع action `notification_settings` — يعيد `push_allowed` و`channels` واتجاه التنقل |
| **إصلاح تظهير الدفعات** | `trg_payment_approved`: القبول (sts=1) ← `i_rd=0` غير مقروء ينوّر ✅ / الرفض (sts=2) ← `i_rd=1` مقروء لا ينوّر ✅. استُبدل نداء `notify_user` بإدراج مباشر للتحكم بـ `i_rd` |
| **الدفع للريبو** | commit `b587168` — ملف migration + 2 تريغرز + 3 دوال محصّنة |
| **إحصاء كلي** | 180 دالة SQL (167 SECURITY DEFINER)، 35 Edge Function، 15 جدول عام نشط |

### 🐞 بَغات/عقبات انصيدت
- **استمرار DUPLICATE_APPOINTMENT**: مواعيد قديمة نشطة من اختبارات سابقة → أُلغيت يدوياً قبل الحجز الجديد.
- **انعدام IPv6 في الساندبوكس**: PostgreSQL في Supabase يستقبل IPv6 فقط → حل: `supabase db query --linked` عبر Management API بدل الاتصال المباشر.
- **`supabase db push`**: يحاول تطبيق كل الميغريشنز من الصفر ولا يمررها → حل: `supabase db query --linked -f file.sql` ينفذ مباشرة.
- **الـ API Management (PAT)**: لا يقبل `pg_get_functiondef()` والاستعلامات الطويلة → حل: `supabase db query --linked` للتحديثات الكبيرة.

### 📚 أصول جديدة
- `supabase/migrations/2026_07_28_payment_notification_unread_fix.sql` — إصلاح تظهير الدفعات.
- `docs/CURRENT_STATUS.md` – مُحدّث بإنجازات 2026-07-28.

### ⚠️ على المالك عند الإطلاق
- ريسيت كامل للبيانات + حساب أدمن جديد للإطلاق (كما قرر المالك).
- حذف توكن GitHub المؤقت + جلسة أدمن يتيمة واحدة (تموت مع الريسيت تلقائياً).
- **تشغيل `flutter run`** لاختبار شاشة push-channels الجديدة.
- **`supabase db dump`** لإقفال انحراف الريبو↔سيرفر (يحتاج Docker).

---

## ✅ سلسلة Push — مُصلحة ومُثبتة حياً (commit `93f18ad`)
- الجذر كان client يكتب `user_devices` مباشرةً (محظور RLS ⇒ صفر أجهزة). الإصلاح: `register_fcm_token`/`unregister_fcm_token` عبر `user-account` · قفل `send-push-notification` بسر `x-push-secret` (جدول `internal_config` غير قابل للمنح) · سبب الرفض الحقيقي بالإشعار · إزالة التكرار من `admin-payments`.
- **الدليل الحي:** وصل البوش على TECNO والتطبيق مسكر نهائياً بالسحبة (HiOS) ✅ وإشعار الرفض وصل بنص السبب ✅.

---

## 🌙 جولة 2026-07-27 — الاختبار الشامل عن بُعد (الوكيل نفّذ بالكامل، المالك وافقات لفظية فقط)

### ✅ مُثبت حياً بالأدلة (تفاصيل التوقعات: `docs/E2E_TEST_PLAYBOOK.md` §5)
| المسار | الدليل |
|---|---|
| رفض دفعة بسبب (مسار كامل) | `sts=2` + `appr_by` + `meta.reject_reason/reject_ts` + **وصل البوش فعلياً على الجهاز** ✅ |
| اعتماد باقة (approve v3) | فضي: `quota=15`, `until=+45d` تمام |
| **التراكم** (ذهبي فوق فضي فعالة) | `pkg=2`, `quota=55` (40+15 بالـ xoff), `until=النهاية+60d`, `stacked/upgraded=true` |
| اعتماد ممولة (كان مكسوراً!) | `until=+7d` + `fms_end` بالعرض مطابق |
| قناة سيرياتل كاش | تعمل بعد إصلاح `payments_channel_check` (كانت ترفض بالقيد) |
| دورة الجلسات | login ← استخدام ← revoke ← `INVALID_SESSION` ✅ |
| التحصين | 172 دالة: 147 كانت محصنة + سد ثغرة PUBLIC بدالة الرفض + قفل 8 helpers ⇒ **الكل `42501`** |
| الخصوصية | `get_user_full_by_id`/`user_payments`/بوابات الإيدج كلها ترفض anon بلا جلسة ✅ |

### 🐞 بَغات انصيدت وأُصلحت بنفس الليلة
1. دالة الرفض الجديدة (3 باراميترات) كانت قابلة للتنفيذ anon عبر مسار `PUBLIC` المدمج (السحب من anon/authenticated لحاله لا يكفي لدالة جديدة) → **ثلاثي التحصين** صار قاعدة دستورية.
2. `payments_channel_check` لم يكن يشمل `syriatel_cash` (كشفها إنشاء دفعة اختبار) → قيد شامل (مايغريشن `2026_06_06` محدّثة).
3. 8 دوال helpers (validators/normalizers) مفتوحة anon «بقرار تاريخي» → أُقفلت لـ service_role بعد موافقة المالك.

### 📚 أصول جديدة
- `docs/E2E_TEST_PLAYBOOK.md` — ملف الاختبار الشامل دوراً-بدور (منهجية + مصفوفات + أدلة مرجعية).
- قسم «بروتوكول عمل الوكيل عن بُعد» في `DEVELOPMENT_GUIDELINES.md` (يعلو على استعلام-واحد-لكل-رسالة قديماً).
- `supabase/functions_dump.sql` مصحّح يدوياً لحقيقة 2026-07-27 (v2/v3/xoff + ختم منح).
- أرشفة لاصقات الليلة المنفذة: `docs/archive/executed_sql_2026-07-27/`.

### 🛡️ سحب RLS الجداول (2026-07-27 — تكميل سحب الدوال): **سليمة، بلا استغلال حي**
- **المكشوف:** 19 جدول → anon يقرأ فقط `offers` (مصمم عام) و`app_config` (بدون أسرار — مفتاح `fcm` فيه URL+anon_key فقط، لازم للتريغرات الداخلية).
- **INSERT:** 17/18 ممنوع `42501`؛ `ratings` يقبل الإدراج بصلاحية الجدول لكن **الـ trigger يحرسه** («تقيّم فقط من تعاملت معه») — التطبيق لا يدرج مباشرة أبداً.
- **UPDATE/DELETE:** صلاحية المنح موجودة لكن السياسات **per-command** ⇒ anon يعدّل/يحذف **صفر صفوف** حتى بالجداول المقروءة (أُثبت بـ PATCH idempotent على app_config وoffers ⇒ `[]`).
- **Drift موثّق:** جدول `social_publications` غير موجود بالحي (PGRST205) رغم وجوده بالمايغريشنز — أُسقط/أُعيدت تسميته قديماً؛ بلا أثر وظيفي حالياً.
- **تحصين وقائي مقترح (بأمر المالك):** `REVOKE INSERT,UPDATE,DELETE` من anon/authenticated على الجداول كعمق دفاعي (عرضة صفر حالياً بفضل RLS).

### 🌙 جولة مسائية بنفس اليوم — مفضّلة/نقاط/فلاتر/ثيم/موظفو اختبار (commits `07bef3b`..`1fd07e5`)
- **قلوب المفضلة:** كانت محلية Hive تُقرأ مرة وحدة → `favoritesListenable` وتحديث حي بالبطاقة والتفاصيل وشاشة المفضلة · إزالة خصم −10 الحقيقي عند إلغاء الإعجاب من المكانين.
- **قفل سكة النقاط:** `award_points` بالإيدجين صار whitelist من `app_config(main).pts` — العميل لا يمرّر رقماً، `manual_add` للإدارة بدور ≥5 وإلا 403 · حذف `rating_bonus` عميلياً (يبقى فقط تريغر SQL).
- **وهم «+5»:** user-rewards كان يلف نتيجة RPC بـ success دائماً → تمرير الحالة كما هي + `AwardResult` + رسالة «بلغت الحد الأقصى اليومي … النقاط ترجع بكرا 🌙».
- **16 موظف اختبار** (2 لكل دور: مصور/مشرف/مكتب/نائب/محامي/معقب + مدير/سمسار بالترقية) كلمة `12345678` — يموتون كلهم بريسيت الإطلاق.
- **شاشة التفاصيل أُعيد تثيمها بالكامل للفاتح — صفر `Colors.white` للنصوص** · **فلاتر الرئيسية الكاملة:** زر ذهبي «فلاتر» بارز حيال البحث + عدّاد فعّالة + معايير المطابقة كاملة (عقارات وسيارات) + ترتيب.

### 📅 إصلاح جذري لبلاغ «لا توجد مواعيد متاحة حالياً» (2026-07-27)
- **الجذر (بدليل من البيانات الحية):** ليس غياب المشرف — المشرف يُختار **وقت الحجز** داخل `book_appointment_internal` (ولو غاب لطلع خطأ مختلف). الجذر أن `avl` فارغة: `شقة فاخرة={"mon":[],"thu":[]}` وعروض أخرى `{}`.
- **خللان:** ① `_buildAvl()` بالشاشتين كانت تكتب اليوم المفعّل حتى لو كل فتراته ناقصة ⇒ `{"mon":[]}` (زر الحجز مفعّل لأن avl «مو فارغة» بس ما في فترات) ② الإضافة بلا افتراضي ⇒ تخطي القسم يحفظ `{}`.
- **الإصلاح:** ① «جاهز بأي وقت» مفعّل افتراضياً بشاشة الإضافة (قرار المالك — التخصيص بالأيام اختياري بطفّي السويتش) ② `_buildAvl()` لا تكتب يوماً بلا فترة مكتملة أبداً ③ تحقق إلزامي قبل النشر/الحفظ: رسالة واضحة وترجيع لخطوة التوفر (عقارات) ④ تنظيف لونين أبيض-ع-أبيض بنفس الشاشتين.
- **تصحيح البيانات (مرة واحدة):** `docs/archive/2026_07_27_fix_empty_avl_booking.sql` — كل عرض حي بلا فترة فعلية ⇒ `{"any":["00:00-23:59"]}` — idempotent: معاينة ← تعديل ← تحقق. (**نُفذ من المالك بنسخة `{"any": true}` — الداتا الحية 3/3 سليمة، ما عاد فيه لزوم**)
...
### 🎛️ حزمة «ابدأ» (2026-07-27) — ٣ قطع منفذة ومدفوعة
1. **⬅️ زر رجوع صريح بشاشة البحث:** الوصول عبر التنقل السفلي (`context.go`) كان يمسح الستاك فلا يظهر السهم ⇒ `leading` صريح: `canPop → pop` وإلا `go('/home')`.
2. **🎛️ فلاتر الزائر الكاملة:** استُخرجت الفلاتر من `user_home_screen` لودجت مشترك `lib/widgets/offer_filters.dart` (`OfferFiltersData` + `OfferFiltersButton` + `showOfferFiltersSheet`) — شاشة الزائر صارت فيها نفس الزر الذهبي بباج العدد ونفس الشيت والمعايير حرفياً، وشاشة المستخدم حُذف منها ~385 سطر مكرر بلا تغيير سلوك.
3. **🛡️ بارسر avl متسامح (`_parseAvl`):** أي قيمة غير-قائمة تُتجاهل أو تؤول (`{"any": true}` ← «جاهز بأي وقت») بدل ما تكسّر جلبة العروض كاملة — سدّ فئة العطب اللي أسقطت التطبيق اليوم كلياً.

### 🔧 حزمة «ابدأ بين ما اختبر» (2026-07-27) — قناتان كانتا ميتتين، أُحييتا
1. **قناة تعديل/حذف العرض:** كانت `updateOffer` تعمل PATCH مباشر بـ anon ⇒ `42501` صامت/فشل منح أن التعديل لا ينحفظ إطلاقاً. **الإصلاح:** أكشنز جديدة بإيدج `user-offers`: `update_offer` (جلسة + `NOT_OFFER_OWNER` للغرباء + قفل المحجوز/المكتمل `OFFER_LOCKED` + whitelist صارمة — sts/i_pub/ts_pub تُفرض سيرفرياً بالمراجعة) و`delete_offer` (i_del=1)؛ `OfferProvider.updateOffer/softDeleteOffer` تحوّلتا للإيدج وتحقن الجلسة تلقائياً.
2. **منحة +500 لصاحب العرض عند الاعتماد (addO):** لم تصل يوماً (واجهة الإدارة تنادي user-rewards بمعرّف المستفيد ≠ المانح ⇒ AUTH_TOKEN_REQUIRED مُبتلع). **الإصلاح:** تريغر DB `trg_offer_approved_award_points` (مايغريشن `2026_07_27_offer_approve_award_points.sql`) — يمنح عبر `award_points_safe` عند أي انتقال حقيقي لـ sts=2 بأي مسار، النقاط من `pts.addO` بالكونفغ، وحد يومي 3 يقطع حلقة رفع-تعديل-إعادة اعتماد. رباعي تحصين كامل.

### ⚠️ على المالك عند الإطلاق
- ريسيت كامل للبيانات + حساب أدمن جديد للإطلاق (كما قرر المالك).
- حذف توكن GitHub المؤقت + جلسة أدمن يتيمة واحدة (تموت مع الريسيت تلقائياً).
- آخر لاصقة SQL (قفل الـ 8 helpers) **نُفذت وتُحققت: 8×42501** ✅ — لم يتبقَ SQL معلّق.
...
| جذر فشل إضافة الطلب الجديد (2026-07-26 — مكشوف عبر اختبار BEGIN/ROLLBACK بالسيرفر) | خطأ 42804: COALESCE(text, jsonb) — الدالة الحية لـ create_request_internal انحرفت يدوياً (p_request->>'specs' بدل ->) فكل إنشاء طلب ينكسر 100%؛ الريبو والـ dump سليمان؛ الحل: REPLACE حي بالنسخة الرسمية من setup.sql + تحقق position/->>'specs' قبل وبعد؛ فحص شامل: لا دوال أخرى بنفس النمط |
| حظر الإبلاغ عن المحتوى الخاص CANNOT_REPORT_OWN (2026-07-26 — إكمال بند حظر الذات الموافق عليه) | سيرفر: create_report_internal بتحققين (tgt_uid=المبلِّغ + tgt_id يشير لعرض يملكه) يرمي CANNOT_REPORT_OWN؛ عميل: زر 🚩 يُخفى كلياً عن عروض المالك + رسالة ودية عند الالتفاف؛ يُطبَّق لايف مع دفعة الإعلان المميز |
| ✅ توثيق لايف شامل 2026-07-26 (بروتوكول الاستعلام المركّب — نتائج المالك) | كل البنود السيرفرية مطبّقة ومتحققة: v1 تجديد تراكمي t، v2 توثيق tp=4 t، v3 إضافة طلب t (الباگ التاريخي مُغلق)، v4 meta=1 + approve + notify t/t، v5 حظر تبليغ ذاتي t، عرض مرمّم 2026-09-24؛ سموكان صفر-كتابة: FORBIDDEN + CANNOT_REPORT_OWN سطر 13. قرار مؤرشف: الوسيط = هوية موثقة بالتعريف (role=1) فلا gating إضافي للحصة 5 |
...
- `2026_07_28_payment_notification_unread_fix.sql` — تظهير الدفعات: القبول ينوّر (i_rd=0)، الرفض لا ينوّر (i_rd=1)
- `2026_07_27_appointments_notifications_pack.sql` — رزمة إشعارات المواعيد الثلاثية (قبول غني + مشرف + تذكيرات)
- `2026_07_27_fcm_push_fix.sql` — إصلاح سلسلة البوش FCM
- `2026_07_27_offer_approve_award_points.sql` — منحة +500 لصاحب العرض عند الاعتماد
...
### إغلاق يوم 2026-07-26 (جلسة الاختبار الحي على جهاز المالك) — ملخص نهائي
- **السيرفر أخضر بالكامل:** إصلاح إضافة الطلب (drift بـ create_request_internal)، التجديد التراكمي، «تمديد/تجديد»، استثناء مشاهدات المالك، منع التبليغ عن الذات، الإعلانات المميزة المدفوعة (tp=1 + approve_payment_final + إشعار) — كلها موثقة بفحوص حية ناجحة.
- **إصلاح ثغرة الإعجاب الذاتي (commit 9f48ad9):** شاشة تفاصيل العرض كانت بلا حماية ولا تمرّر offer_id — أُغلقت (منع + تعتيم القلب + تمرير offer_id)، وتحصين سيرفري: user-rewards يرفض أي like بدون offer_id (OFFER_ID_REQUIRED). الدالة منشورة (تأكيد المالك).
- **مركز بيع الإعلانات (commit 3d8083d):** دمج داخل شاشة الباقات (قرار المالك — دخول من «حسابي» بلا تبويب سفلي): سلّم أسعار من config.fmsp + اختيار من العروض المنشورة (sts=2) + تابع للدفع. الشاشة صارت «الباقات والإعلان المميز».
- **تنظيف نقاط الاختبار الفاشل:** عدادات user_daily_limits كشفت 7 likes (+70) و3 manual_add (−30) لحساب مستخدم1عادي (0e1a0296) → صافي +40 غير مستحق → صُحّح بـ UPDATE واحد: pt من 790 إلى 750 (نفّذه المالك 2026-07-26 صباحاً). حساب المالك pt=0 نظيف.
- **مؤجّل بقرار المالك (بعد استيقاظه):** اختبارات حية متبقية — الإعجاب بعروض الغير (+10 يجب أن تعمل)، بطاقة «باقتي»، مصطلحات تمديد/تجديد، شعار الشركة وهاتف المطوّر الثاني، شراء إعلان مميز E2E من التبويب الجديد. وميزة مؤجلة: فيديو واتساب للعروض.

### مصفوفة API عن بُعد 2026-07-26 مساءً (جلسة اختبار آلية — صفر تدخل من المالك)
- فحوص بلا جلسة: سلامة تواريخ العروض ✓، عمود payments.meta ✓، جدار 401 قبل أي منح ✓، تحقق المدخلات وجدار الدخول ✓، الكونفيغ (fmsp غائب=افتراضي 50/95/135/180k، كل قنوات الدفع مفعّلة) ✓.
- فحوص بجلسة الحساب التجريبي (API): إعجاب ذاتي → SELF_ACTION ✓ · مشاهدة ذاتية → OWNER_VIEW skip ✓ · عكس نقاط −10 ✓ · الرصيد متماسك 750 ✓.
- **عطل إجرائي اكتشف وأُصلح:** user-rewards كانت منشورة من نسخة محلية قديمة (نشر قبل pull) → الإعجاب بلا offer_id كان يمنح! الحل: pull ثم deploy ← أُعد الفحص عن بُعد → OFFER_ID_REQUIRED ✓. **قاعدة إلزامية مستقبلاً: git pull دائماً قبل أي deploy.**
- اختبار «إعجاب بعرض الغير» موقوف هيكلياً: لا توجد عروض منشورة لغير الحساب التجريبي بقاعدة البيانات — يُنفذ عن بُعد فور توفر عرض ثانٍ.
- **المنهج الجديد:** كل الاختبارات المنطقية أصبحت تُنفذ عن بُعد عبر API بلا حاجة لجهاز المالك — المتبقي لديه مجرد جولة بصرية اختيارية (باقتي، تمديد/تجديد، الشعار).
