# الحالة الحالية للمشروع — المكتب العقاري الالكتروني

> آخر تحديث: 2026-06-11
> المستودع الصحيح: `hady-albnnai/sweeda-real-estate-office-elc`

## ملخص تنفيذي

المشروع حالياً أصبح تطبيق مكتب عقاري إلكتروني مبني على Flutter + Supabase، مع:

- واجهة زائر ومستخدم.
- لوحة وسيط.
- لوحة إدارة.
- نظام صلاحيات داخلي ديناميكي.
- مركز عمليات المكتب.
- إدارة مهام التصوير والمصور.
- إدارة الوسائط.
- حماية مسارات حسب الدور والصلاحية.
- دعم وضع التطوير الحالي لتسجيل WhatsApp OTP fallback.
- تنفيذ دفعة إصلاحات منطقية كبيرة موثقة في `docs/LOGIC_REPAIR_TRACKER.md`.
- تطبيق دفعة 40 دالة RPC على السيرفر وتحققنا من وجودها كاملةً بتاريخ 2026-06-11 (migration: `2026_06_11_real_test_stabilization_internal_rpcs.sql`).
- الكود والسيرفر الآن متوافقان — كل المسارات الحرجة تعتمد RPCs.
- المتبقي الوحيد غير المغلق هو `S-01` (تشديد نموذج المصادقة الكامل عند الانتقال من dev fallback إلى Auth إنتاجي حقيقي) — مؤجل بقرار.
- حماية pkg_grace في check_user_safe_update trigger + user_details_screen تعرض حالة الباقة.
- grace_days من Config (app_config.pkg.grace_days=3) + ConfigModel.pkgGraceDays getter + packages_screen يعرضها ديناميكياً.
- إصلاح مراجعة الباقات: amt حُذف من URL + زر جدّد يذهب لصفحة الدفع + dynamic→ConfigModel? + profile_screen يعرض grace period.
- نظام الباقات الاحترافي (2026-06-12 Batch 2): Grace Period (3 أيام) + send_renewal_reminders + منع دفعة مزدوجة + سعر من Config + my_payments_screen + payments_screen تعرض السعر المفترض
- إصلاح نظام الباقات كاملاً (2026-06-12 Batch 1): purchase_offer_boost + create_offer_internal + expire_packages cron + canPublishOffer/offerQuota تفحصان pkgEnd + packages_screen تعرض "انتهت الباقة" + payments_screen تعرض اسم المستخدم.
- إصلاح streak اليومي: v_yesterday + تصفير عند الانقطاع.
- إصلاح 6 مشاكل مكتشفة: updateOffer بلا ts_upd + bookAppointment بلا ownerId + broker لا يجلب بيانات الطالب + edit_offer_screen كامل + addedBy يعرض اسم الموظف + appointments_management تعرض المشرف.
- تطبيق دفعة إدارة الطلبات (2026-06-11 Batch 3): `get_admin_requests_internal` + شاشة `/admin/requests` + إصلاح `add_request_screen` كاملاً.
- إصلاح شاشة إضافة الطلب: تصنيف من Config + عملة + specs + خصوصية بيانات العميل.
- إعادة هيكلة الأدوار (2026-06-12): 7 أدوار بدل 5 — مستخدم/وسيط/مصور/مشرف/موظف مكتب/نائب مدير/مدير. **مُطبّق على السيرفر.**
- نظام المنفذ الميداني (2026-06-12): شاشة مهام (4 تبويبات) + شاشة تنفيذ (3 إجراءات) + جدول completion_requests + 7 RPCs + شاشة مراجعة طلبات الإتمام للمكتب. **مُطبّق على السيرفر.**
- تدقيق عميق (2026-06-12): approve_payment role>=5 + create_offer إعفاء role>=4 + 11 RLS policy + المصور لا يرث broker + المشرف لا يصل admin + شاشة موظف مكتب مخصصة. **مُطبّق على السيرفر.**
- إصلاح شامل لحجز المواعيد ودورة حياة العرض (2026-06-12): book_appointment 10 فحوصات (avl + تعارض + منفذ + طلب إتمام) + offers.sts يتحدث تلقائياً (5=محجوز، 6=مكتمل) + إلغاء مواعيد عند إتمام صفقة + عدد المواعيد النشطة + 5 دوال role محدّثة. **مُطبّق على السيرفر.**
- شاشة إضافة عرض احترافية (2026-06-13): فصل حقول العقار عن السيارة + مناطق (السويداء/صلخد/شهبا) + حقول عقار (مساحة/إكساء/طابق/اتجاه/ملاحظات قانونية) + حقول سيارة (لوحة/محافظة/ماركة/موديل/سنة/لون/كم/وقود/ناقل) + سند سيارة + نوع نمرة + عنوان تلقائي ذكي.
- تفاصيل العرض احترافية (2026-06-13): عرض كل specs حسب النوع بأيقونات + معلومات سريعة ببطاقة العرض + رقم العرض (#) + إخفاء المواعيد للسيارة.
- تحسينات عامة (2026-06-13): سبلاش محدّث + إشعارات "العرض الخاص بك" + رقم عرض تسلسلي + نقاط فقط عند مشاركة فعلية + checkbox سوشال + فلتر مطابقة (سعر/موقع) + زر حجز من بطاقة المطابقة.
- **التطبيق جاهز للاختبار الحقيقي الشامل.**

## اسم التطبيق المثبت

```txt
المكتب العقاري الالكتروني
```

مع استمرار وجود هوية/منطقة عقارات السويداء داخل المحتوى الحالي، ليبقى التطبيق قابلاً للتوسع لاحقاً لكل سوريا.

## الأدوار الحالية

يعتمد المشروع على `users.role`:

| role | الدور | الوصف |
|---:|---|---|
| 0 | مستخدم | مسجّل عادي |
| 1 | وسيط | مستخدم ترقّى لوسيط |
| 2 | مصور | موظف داخلي — تصوير العروض |
| 3 | مشرف | موظف ميداني — ينزل مع الزبائن |
| 4 | موظف مكتب | عمليات مكتبية |
| 5 | نائب مدير | |
| 6 | مدير | أعلى صلاحية |

> الزائر ليس دوراً في قاعدة البيانات — هو حالة بدون تسجيل دخول.

### حدود الصلاحية في RPCs
- `role >= 3` (مشرف): عمليات إدارية عامة (مراجعة عروض، مواعيد، تقارير...)
- `role >= 5` (نائب مدير): تغيير أدوار وصلاحيات المستخدمين
- `role >= 6` (مدير): إعدادات التطبيق

## الصلاحيات الديناميكية

تمت إضافة الحقل:

```sql
users.perm JSONB DEFAULT '[]'
```

القاعدة:

- `perm = []` → يستخدم التطبيق صلاحيات الدور الافتراضية.
- `perm` غير فارغة → تستخدم كصلاحيات مخصصة.

أهم الشاشات:

| الشاشة | المسار | الصلاحية |
|---|---|---|
| لوحة الإدارة | `/admin/dashboard` | role >= 3 (مشرف) |
| عمليات المكتب | `/admin/office-operations` | `office_operations` |
| إدارة الصلاحيات | `/admin/permissions` | `manage_permissions` |
| إدارة الوسائط | `/admin/media-review` | `media_review` |
| إدارة مهام التصوير | `/admin/photography-management` | `photography_management` |
| مهام المصور | `/photographer/tasks` | `photographer_tasks` |

## الإدارة الداخلية

تم تنفيذ:

- حماية مسارات الإدارة والوسيط والمستخدم والمصور.
- مركز عمليات المكتب.
- شاشة إدارة الصلاحيات.
- شاشة إدارة الوسائط.
- شاشة إدارة مهام التصوير.
- شاشة مهام المصور.
- إصلاح تغيير دور المستخدم وحالته عبر RPC.
- منع تكرار رقم الهاتف بصيغ مختلفة.
- إعفاء حسابات الإدارة `role >= 2` من قيود حصة العروض.
- إنشاء العرض عبر RPC لتفادي مشاكل RLS في وضع التطوير.

## المصور

تم تنفيذ نظام مستقل:

- جدول: `photography_tasks`
- Provider: `PhotographyProvider`
- Model: `PhotographyTaskModel`
- شاشة المصور: `/photographer/tasks`
- شاشة الإدارة: `/admin/photography-management`

حالات المهمة:

| sts | الحالة |
|---:|---|
| 0 | بانتظار المصور |
| 1 | قيد التنفيذ |
| 2 | مرسلة للمكتب |
| 3 | معتمدة |
| 4 | مرفوضة |
| 5 | ملغاة |

## وضع المصادقة الحالي

يوجد WhatsApp OTP مع Edge Functions، لكن يوجد أيضاً dev fallback.

هذا يعني أن بعض الدوال القديمة التي تعتمد على `auth.uid()` قد تفشل في وضع التطوير. لذلك أُضيفت دوال إدارية متوافقة مع الوضع الحالي وتقبل `p_admin_uid` مع فحص دوره من جدول `users`.

## الملفات المرجعية التي يجب الاعتماد عليها

| الملف | الغرض |
|---|---|
| `README.md` | نقطة الدخول للمشروع |
| `docs/CURRENT_STATUS.md` | الوضع الحالي المختصر |
| `docs/NEXT_DEVELOPMENT_ITEMS.md` | المتبقي فقط |
| `docs/SPEC.md` | مواصفات النظام والبيانات |
| `docs/LOGIC_SPEC.md` | ميثاق المنطق الأعلى |
| `docs/LOGIC_AUDIT_2026_06_10.md` | تقرير التدقيق المنطقي المرجعي |
| `docs/LOGIC_REPAIR_TRACKER.md` | متتبع إصلاح المنطق بين المحادثات |
| `docs/POST_FIX_EXECUTION_AND_TEST_PLAN.md` | خطة التنفيذ والاختبار بعد الإصلاحات |
| `DEVELOPMENT_GUIDELINES.md` | قواعد التطوير الإلزامية |
| `supabase/FUNCTIONS_REFERENCE.md` | مرجع دوال السيرفر |
| `supabase/CHECK_ALL_MIGRATIONS.sql` | فحص حالة السيرفر |
| `docs/AUTH_SETUP.md` | إعداد المصادقة |
| `docs/WHATSAPP_ACTIVATION_PLAN.md` | تفعيل واتساب الإنتاجي |
| `docs/SECURITY_REVIEW.md` | مراجعة أمنية |
| `BUILD_GUIDE.md` | البناء والنشر |

## ملاحظات للمحادثة القادمة

ابدأ من هنا:

1. المشروع الصحيح هو `sweeda-real-estate-office-elc`.
2. لا تعيد فتح تفاصيل المستودع الخاطئ؛ تم عكس تعديلاته.
3. الإدارة والصلاحيات والمصور منفذة ومرفوعة.
4. المرحلة القادمة هي اختبار فعلي وإصلاح أخطاء الاختبار.
5. أي تعديل يجب أن يحدث التوثيق ويرفع للمستودع مباشرة.


## توثيق تغييرات السيرفر والتراجع

راجع:

```txt
docs/SERVER_CHANGES_2026_06_10.md
supabase/ROLLBACK_2026_06_10_INTERNAL_MANAGEMENT.sql
```

## تنظيف اختبارات QA

تم حذف شاشة فحص النظام وملفات الاختبار الآلي من التطبيق والمستودع للحصول على نسخة إنتاجية أنظف.
لحذف دالة الفحص من السيرفر نفّذ:

```txt
supabase/migrations/2026_06_10_remove_qa_system_check.sql
```
