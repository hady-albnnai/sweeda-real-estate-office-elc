...
| المجال | الحالة |
|---|---|
| منع نقاط الإعجاب بالذات + إصلاحات واجهة (2026-07-26 دفعة اختبارات المالك) | **الجدار الخادمي (الفعلي):** حارس `SELF_ACTION` أُضيف لمساري المنح — `user-rewards.award_points` و`user-account.award_points`: الكلاينت يمرر `offer_id` والسيرفر يستخرج مالك العرض بنفسه ويرفض المنح لو كان هو نفسه؛ **العميل:** القلب يتعتّم على عروضك + توست «لا يمكن الإعجاب بعرضك الخاص» قبل أي طلب؛ `awardEvent` صار يقبل `offerId`. **سبلاش:** RenderFlex overflow ~34px على TECNO KI7 (360px) — الشعار صار يتصاغر مع نقص الارتفاع بدل الانكسار. **نص الباقة المجانية:** fallback map بـ packages_screen كان يعرض مجاني=5 عروض → صار 1 مطابقاً للقاعدة. **نشر مطلوب:** `user-rewards` + `user-account` بـ `--no-verify-jwt` |
| بوابة السقف المسبقة + إصلاح بناء (2026-07-26 بطلب المالك) | المنع صار عند الضغط على «إضافة عرض» مباشرةً في «عروضي» (FAB + حالة الفراغ): حوار «تجاوزت عدد العروض المسموح بها» + زر «الاشتراك بباقة» → `/user/packages` — بدل اكتشاف الرفض بعد تعبئة البيانات؛ الحارس السيرفري `create_offer_internal` يبقى الفيصل (نفس القواعد: sts المحسوبة، إعفاء role≥4، fallback، pkg الفعالة). وأُصلح خطأ بناء كان سببه هروب `\$` ناقص بسطر priceStr |
| حصة العروض المجانية — سبب «النشر بلا حدود» (2026-07-26) | الجذر **بيانات لا كود**: قيمة يدوية حية `pkg.0.o=5` كانت تعطي المجاني 5 عروض (الدالة تلتقطها قبل fallback user=1/broker=5). الإصلاح: حذف مفتاح `"0"` من `pkg` (`value #- '{pkg,0}'`) فيُفعّل الـ fallback الأصلي؛ الـ seed صار يوثّق ذلك قصداً (pkg بلا "0") مع توضيح سبب الغياب |
| رابط التطبيق بمنشورات السوشال (2026-07-26 بطلب المالك) | المفتاح `txts.appDownloadLink`: حقل جديد بمحرر الإعدادات (يُحفظ عبر admin-config بعد توسيع قائمتها البيضاء) + المولدان يدرجانه في المنشور **فقط إذا عُبّئ** — فارغ الآن بانتظار نشر المتجر؛ وهذا متوافق كلياً مع سياسات المتجر (مجرد نص منشور) |
| مثال هاتف العميل بإضافة طلب (2026-07-26) | ✅ كان الـ hint يعرض رقماً حقيقياً (0938862469) — صار `مثال: 09xxxxxxxx` |
| قالب منشور السوشال الجديد (2026-07-26) | ✅ طُبّق بمولّدين: `BusinessService.generateSocialPost` (تفاصيل/نشر/مطابقة) و`_generateSocialPostText` بالنشر التلقائي — تفاصيل العرض كاملة (تصنيف رئيسي/فرعي من الإعدادات + سعر + منطقة + مواصفات حسب النوع + وصف) + «التواصل حصراً عبر واتساب المكتب» برقم من `txts.videoRequestWhatsApp`، وقاعدة صارمة بالكود: لا معلومات تواصل لصاحب العرض أبداً (`contactPh` يُتجاهل) ولا لوحة/سند بالمنشور |
| 🗂️ مهام مجدولة قادمة (2026-07-26 بطلب المالك) | 1) حظر الإبلاغ عن العرض و/أو الإعجاب به من مالكه نهائياً (عميل+سيرفر) وإيقاف كسب النقاط من ذلك؛ 2) تصحيح مثال هاتف العميل الإلزامي بإضافة طلب → `09xxxxxxxx` (كان يعرض رقماً حقيقياً)؛ 3) تحقيق وإصلاح فشل إضافة طلب جديد المُبلغ عنه |
| رفع صور العروض — الجذر النهائي (2026-07-26) || رفع صور العروض — الجذر النهائي (2026-07-26) | **طبقتان اكتُشفتا بالفحص الحي (curl ضد السيرفر الحقيقي):** 1) بوابة Supabase ترد 401 `UNAUTHORIZED_NO_AUTH_HEADER` لأن `verify_jwt=true` افتراضياً عند النشر — الحل: نشر كل الدوال بـ `--no-verify-jwt` (أُضيف كبروتوكول إلزامي للدستور، التوثيق يبقى داخل الدالة)؛ 2) بعد فتح البوابة ظهر 500: `uploadBinary is not a function` — الدالة كانت تستدعي method موجودة فقط بعميل Dart، أُصلحت لـ `.upload()` القياسية مثل باقي الدوال (`create-user`, `legal-actions`…). تعديلات مصاحبة: تسمية «رقم لوحة السيارة (إلزامي)»، تمرير لأعلى عند الانتقال بين خطوات إضافة العرض (ScrollController + `_goToStep`)، تصنيف سيارات فرعي «سياحية» بدل «سيدان» (بيانات حية + seed) |
| مسار فيديو العروض عبر واتساب (2026-07-26 — مرحلة التحقق) | اتفق التصميم: الفيديو لا يُرفع للسيرفر؛ المالك يرسله **خاصاً** لرقم المكتب مع رمز العرض فقط `#م-128` (بلا بيانات مالك — حماية من التسريب)؛ الموظف يعلّم "يوجد فيديو" والزبون يطلبه بإشعار للمكتب. حقائق حية مؤكدة: `log_admin_action(p_admin_uid, p_act, p_det, p_ref_id, p_ref_col)` ✅؛ **فجوة مؤكدة**: كتابة `app_config` من التطبيق مكسورة بالهاتف (RLS يشترط auth.uid+role≥6 ولا دالة كتابة) ← الحل: `admin-config` جديدة؛ **باغ مكتشف**: نداءات log_admin_action في `admin-offers` بأسماء معاملات خاطئة (p_action…) تفشل بصمت ← سيُصلح مع الحزمة. بروتوكول "استعلام واحد في كل مرة" أُضيف للدستور |
| شاشة إضافة العرض بكل الأدوار (2026-07-26) | تدقيق شامل وإصلاح 8 مشاكل — أهمها **كراش Stepper** المؤكد من logcat: تغيير عدد الخطوات عند اختيار سيارة يرمي assert أثناء البناء فتبقى الشاشة على الفريم القديم (سبب "تصنيفات عقارات + شاشة ميتة")، أُصلح بـ ValueKey؛ أزرار التالي/رجوع كانت مفقودة؛ ودجت `_dd` بلا value فتختفي الاختيارات؛ قراءة آمنة للإعدادات؛ watch بدل read؛ seed رسمي للتصنيفات (الداتا الحية تحقّقت سليمة ✅)؛ مدخل "إضافة عرض" للمدير/النائب؛ توحيد add_offer_admin؛ حقل محافظة للسيارة — التفاصيل: `docs/ADD_OFFER_SCREEN_AUDIT_2026-07-26.md` |
| إصلاح التجديد التراكمي + مصطلح تمديد/تجديد (2026-07-26) | باگ مكتشف من المالك بالاختبار الحي: التجديد المدفوع كان يعيد ضبط الانتهاء من 'الآن' بدل التراكم فوق المتبقي (دفع 500 نقطة بلا فائدة) — الجذر: العروض الجديدة ts_end=NULL مقصود (الانتهاء يُحسب نشر+30 في الإكسبايري والتطبيق) ودالة purchase_offer_boost كانت تبدأ من الآن عند غيابه. الإصلاح: أساس التجديد والمجانية = الانتهاء الفعلي COALESCE(ts_end, النشر/الإنشاء+30) ثم +30 يوم؛ نفس الأساس بشروط المجانية (آخر يومين) التي كانت ستتعطل دائماً لأول تجديد. عميل: «تمديد العرض» مدفوع (+30 فوق المتبقي بأي وقت) / «تجديد مجاني ✨» عند يومين أو أقل + تنويه توضيحي بالشاشة. ترميم بيانات: عرض المالك المجدد ts_end=ts_pub+60 يوم (30 أصلية + 30 مدفوعة) |
| استبعاد مشاهدات المالك من عداد عرضه (2026-07-26 بطلب المالك) | طبقتان: عميل — offer_detail_screen لا ينادي incrementViews إذا المشاهد = المالك (ويرسل viewer_uid)؛ سيرفر — user-offers (increment_views) يقارن viewer_uid مع usr_id ويرد skipped=OWNER_VIEW بدون زيادة. بلا توثيق إلزامي عمداً (الزوّار يُحسبون؛ أسوأ استغلال = تقليل عداد، عديم الجدوى). لا SQL — تحديث عميل + نشر دالة فقط |
| حوار «عن التطبيق»: شعار الشركة + رقم مطور ثاني (2026-07-26 بطلب المالك) | شعار loraneem (1200×520 عريض) كان يُعرض بمربع 80×80 مع BoxFit.cover فيُقتص ثلثاه → contain بعرض 150. رقم مطور ثانٍ 0938862469: getter جديد developerPhone2 (افتراضي بدون SQL، نص فارغ يخفيه) + سطر بالحوار + حقل إداري بشاشة الإعدادات |
| إزالة تكرار «خدمة التصوير العقاري» من بروفايل المستخدم (2026-07-26 بطلب المالك) | حُذف عنصر القائمة من profile_screen — الشاشة المخصصة تبقى عبر تبويب «تصوير» بالشريط السفلي فقط (مصدر وحيد للميزة) |
| بطاقة «باقتي» بمعلومات الحساب (2026-07-26 — تصميم وافق عليه المالك) | بدل سطر نصي بأسماء ثابتة (فضي/ذهبي): بطاقة مستقلة تقرأ اسم الباقة من app_config، شريط حالة (فعّالة/سماح/منتهية/مجانية)، حصة العروض المستهلكة X/Y بشريط تقدم (مرآة منطق create_offer_internal: المشترك من باقته، الوسيط 5، المجاني من pkg.0)، وزر ترقية/تجديد يفتح شاشة الباقات |
| الإعلان المميز المدفوع v1 (2026-07-26 — قرارات المالك: بالمصاري، خصم عالأطول، إلغاء نسخة النقاط) | مدة 1-4 أسابيع بأسعار فئوية (افتراضي 50/95/135/180 ألف ل.س، قابلة للتعديل من fmsp بالإعدادات)؛ شراء من شاشة ترقية العرض عبر مسار الدفع اليدوي الموجود (قناة+إثبات+مرجع)؛ سيرفر: عمود payments.meta (jsonb) + create_payment_internal بتحقق الملكية/المدة/منع التكرار + approve_payment_final بفرع tp=1 يفعّل i_fms/fms_end تراكمياً + trg_payment_approved بنص مخصص؛ بطاقة fms بالنقاط أُزيلت من الواجهة |
| توثيق الهوية (2026-07-25) | إصلاح جذري لفشل رفع صورة الهوية: الرفع نُقل من العميل المباشر (محظور بـ RLS) إلى Edge Function `user-account` action جديد `upload_id_images` بصلاحية service_role؛ الشاشة تدعم صورتي الهوية (أمامي/خلفي) واختيارهما دفعة واحدة؛ حذف SnackBar التنبيه المربكة والتوجيه مباشرة لشاشة الهوية مع تنويه ثابت |
| خدمة التصوير العقاري (2026-07-25) | شاشة مستقلة `/user/photography` (شرح الخدمة + نموذج طلب + قائمة طلبات المستخدم وحالتها عبر action جديد `my_photo_requests` في `admin-photography`) متاحة للمستخدمين والوسطاء |
| إغلاق حلقة التصوير (2026-07-25) | إصلاح 3 فجوات حرجة: رؤية المكتب للطلبات عبر `list_tasks` (كانت فارغة بسبب RLS)، إسناد مصور للطلبات بلا عرض عبر `assign_photographer` + زر إسناد بالإدارة، اعتماد الطلبات بلا عرض عبر update_status، وعرض الوسائط المعتمدة للمستخدم |
| تحسينات التصوير (2026-07-25 ج3) | إشعارات فورية لدورة التصوير (طلب جديد→المكتب، إسناد→المصور، نتيجة→المالك، إلغاء→المصور) + زر إلغاء الطلب للمستخدم (sts=0 فقط) عبر `cancel_photo_request` + تنظيف setup.sql (توثيق الجدول وسياساته، تصحيح RPCs قديمة، إضافة user_photography للقائمة البيضاء) |
| رفع صور العروض (2026-07-25 ج4) | إصلاح جذري لتعليق "جاري رفع الصور" اللانهائي عند إضافة عرض: السبب رفض الجلسات العادية بـ upload-offer-images (min_role=2) + استثناء غير ملتقط بالعميل بلا مهلة زمنية؛ الآن مستخدم الهاتف يرفع لمجلده فقط، والعميل يوقف المؤشر ويعرض الخطأ الحقيقي، مع بطاقة تحميل أنيقة |
| الشريط السفلي للمستخدم (2026-07-25) | 6 تبويبات: الرئيسية، عروضي، طلباتي، مواعيدي، تصوير، حسابي — "عروضي" أصبحت قابلة للوصول للمستخدم العادي (كانت يتيمة)، زر إضافة عرض نُقل من أيقونة أعلى الرئيسية إلى FAB داخل "عروضي"، والمفضلة انتقلت لأيقونة قلب أعلى الرئيسية |
| تفاصيل الإصلاحات | `docs/USER_VERIFICATION_PHOTOGRAPHY_FIXES_2026-07-25.md` |
...
| تنظيف النظام (Wipe) | النسخة 2 (2026-07-26): سكربت `supabase/utils/cleanup_test_data.sql` مُوسّع — يصفر 20 جدولاً + auth.users اليتيمة + storage.objects + كل العدادات، داخل معاملة ذرّية، ويُبقي فقط حساب المدير وجلساته وapp_config كاملة (النسخة 1 نُفذت 2026-06-16) |
...
## تحديث 2026-07-04 — فحص وإكمال دورة مهام المحامي ↔ المعقب

- تم فحص مسار إرسال مهمة من المحامي إلى المعقب ووجدت نواقص في دورة الإتمام:
  - كانت المهمة تصل للمعقب وتظهر في قائمته.
  - كان المعقب يستطيع تحديث بنود checklist فقط.
  - لم يكن هناك زر واضح لتأكيد أن المهمة اكتملت بالكامل.
  - لم يكن هناك إشعار للمحامي عند اكتمال المهمة.
- تمت إضافة دورة كاملة:
  1. عند إنشاء المهمة يتم إشعار المعقب (`expediting_task_assigned`).
  2. عند بدء تحديث أي بند تتحول المهمة إلى `status=1` قيد الاستخراج.
  3. بعد اكتمال كل البنود يظهر زر **إتمام المهمة وإشعار المحامي** في تفاصيل مهمة المعقب.
  4. السيرفر يرفض الإتمام إذا بقي أي بند غير مكتمل (`CHECKLIST_NOT_COMPLETE`).
  5. عند إتمام المعقب تتحول المهمة إلى `status=2` ويُرسل إشعار للمحامي (`expediting_task_completed`).
  6. في شاشة المحامي تظهر المهمة كـ **بانتظار اعتمادك** مع زر **اعتماد إنجاز المعقب وإشعاره**.
  7. بعد اعتماد المحامي تتحول المهمة إلى `status=3` ويُرسل إشعار للمعقب (`expediting_task_approved`).
- تمت إضافة RPCs محصنة ومقفولة عن العميل المباشر:
  - `complete_expediting_task_internal(uuid, uuid, text)`
  - `approve_expediting_task_internal(uuid, uuid)`
- تم تحديث `legal-actions` بإجراءات:
  - `complete_expediting_task`
  - `approve_expediting_task`
- تم تطبيق Migration على السيرفر الحي: `2026_07_04_expediting_task_completion_flow.sql`.
- تم اختبار دورة SQL كاملة داخل Transaction مع rollback:
  - إنشاء محامي ومعقب تجريبيين.
  - إنشاء مهمة وإشعار المعقب.
  - محاولة إتمام قبل اكتمال البنود وفشلها كما يجب.
  - إكمال جميع البنود.
  - إتمام المهمة وإشعار المحامي.
  - اعتماد المحامي وإشعار المعقب.
  - تم التراجع عن بيانات الاختبار بالكامل.
- الفحص الأمني بعد التطبيق: `open_security_definer_count = 0`.
...
## تحديث 2026-07-05 — إلغاء الوضع الليلي والعودة للنهاري فقط

- تم إلغاء نظام تبديل الوضع النهاري/الليلي بالكامل بسبب عدم اتساق بعض الشاشات مع التبديل الديناميكي.
- تم حذف زر الثيم العائم و `ThemeProvider` وإرجاع التطبيق إلى الوضع النهاري الأبيض/الذهبي فقط.
- تم تعديل شعار شاشة حسابي للزائر بحيث لا يظهر كمربع أسود داخل دائرة؛ أصبح الشعار كاملاً داخل إطار مستدير مع فيد ناعم من حواف الأسود إلى خلفية التطبيق البيضاء للحفاظ على وضوح الكتابة.
- لا يحتاج هذا التعديل أي نشر Supabase لأنه تعديل Flutter فقط.

---

## تحديث 2026-07-05 — تخصيص وثائق مهمة التعقيب من المحامي

- تم تعديل شاشة المحامي عند إنشاء مهمة تعقيب:
  - اختيار نوع المهمة: عقار أو سيارة.
  - اختيار الوثائق المطلوبة من قائمة حسب النوع.
  - إمكانية إضافة كل الوثائق الافتراضية دفعة واحدة.
  - إمكانية إضافة وثيقة أخرى كنص حر.
  - لكل وثيقة يستطيع المحامي تحديد عدد النسخ المطلوبة.
  - لكل وثيقة يستطيع المحامي كتابة تعليمات خاصة.
- لم تعد مهمة التعقيب تعتمد فقط على رقم العقار/المنطقة وتعليمات عامة.
- تم توسيع `ChecklistItemModel` ليحمل:
  - `required_copies`
  - `lawyer_instructions`
- تم تحديث شاشة تفاصيل مهمة المعقب لعرض عدد النسخ وتعليمات المحامي لكل وثيقة بوضوح.
- مسار السيرفر لا يحتاج Migration لأن `create_expediting_task_internal` يقبل checklist مخصصاً عبر `p_checklist` ويحفظه كما هو.

---

## تحديث 2026-07-05 — تحسين اختيار وثائق مهمة التعقيب

- تم تعديل شاشة إنشاء مهمة التعقيب عند المحامي لتدعم تحديد أكثر من وثيقة بنفس المهمة عبر قائمة اختيار متعددة بدلاً من اختيار منفرد.
- تم فصل مفهومين مختلفين:
  - **إضافة وثيقة دائمة للقائمة**: تُحفظ محلياً على جهاز المحامي وتظهر في المهام القادمة ويمكن حذفها من القائمة.
  - **وثيقة لمرة واحدة / نص حر**: تُضاف للمهمة الحالية فقط ولا تُحفظ في القائمة الدائمة.
- تمت إضافة إمكانية حذف الوثائق المضافة للقائمة الدائمة من شاشة المحامي.
- بقيت الوثائق الأساسية الافتراضية محمية من الحذف داخل التطبيق.
- تم إصلاح مشكلة الوثيقة الحرة التي لم تكن تظهر/تثبت بوضوح بعد إضافتها؛ الآن تُضاف مباشرة إلى الوثائق المحددة للمهمة.
- لكل وثيقة محددة يستطيع المحامي تحديد عدد النسخ وكتابة تعليمات خاصة تصل للمعقب داخل تفاصيل المهمة.

---

## تحديث 2026-07-05 — صور سندات التعقيب ومراجعة المحامي قبل الاعتماد

- تم استبدال حقل رابط صورة السند عند المعقب بزر اختيار صورة من المعرض.
- صور السندات/الوثائق المستخرجة تُرفع إلى bucket خاص جديد `expediting_docs` وليست روابط يدوية.
- `legal-actions` يرفع صورة الوثيقة عبر `service_role` ويخزن مسارها الخاص داخل checklist.
- عند جلب مهام المحامي أو المعقب يتم توليد signed URLs مؤقتة لصور الوثائق حتى تظهر داخل التطبيق.
- تمت إضافة `attachment_signed_url` و `revision_notes` إلى `ChecklistItemModel`.
- أصبح المحامي يستطيع فتح تفاصيل المهمة من تبويب المهام المرسلة والاطلاع على:
  - الوثائق المطلوبة.
  - عدد النسخ.
  - تعليمات المحامي الأصلية.
  - بيانات وملاحظات المعقب.
  - صورة كل سند/وثيقة رفعها المعقب.
- قبل اعتماد المهمة، يستطيع المحامي طلب إعادة أي سند غير صحيح مع كتابة سبب الإعادة.
- طلب الإعادة يحول المهمة إلى `status=1` ويلغي `completed_at` ويرسل إشعاراً للمعقب.
- تمت إضافة RPC محصنة ومقفولة عن العميل المباشر:
  - `request_expediting_item_revision_internal(uuid, uuid, text, text)`
- تم تطبيق Migration على السيرفر الحي: `2026_07_05_expediting_documents_review_flow.sql`.
- تم اختبار دورة SQL داخل Transaction مع rollback:
  - checklist مخصص يحفظ `required_copies` و `lawyer_instructions`.
  - تحديث بند مع صورة/مسار.
  - إتمام مهمة.
  - طلب إعادة بند من المحامي.
  - عودة المهمة إلى `status=1` وتثبيت `revision_notes`.
...
## تحديث 2026-07-25 — إصلاحات تسجيل المستخدم والرابط السحري

- تم منع الفراغات في اسم المستخدم داخل شاشة إعداد الحساب عبر حذفها تلقائياً أثناء الإدخال/اللصق، مع فحص محلي لحظي وفحص توفر مؤجل لتخفيف استدعاءات السيرفر.
- تم تحسين رسائل فشل إعداد الحساب بعد Magic Link، وخصوصاً حالات رقم الهاتف المستخدم مسبقاً أو انتهاء/عدم تطابق جلسة الرابط السحري.
- تم تعديل `user-account.handle_email_auth` حتى لا يعيد `user_id` مختلفاً عن JWT عند وجود بريد مربوط بحساب آخر، لأن ذلك يكسر حفظ الملف وكلمة المرور لاحقاً.
- تمت إضافة تنبيهات واضحة للمستخدم لفحص Spam / البريد غير المرغوب فيه عند التسجيل بالإيميل.
- المرجع التفصيلي: `docs/USER_AUTH_FIXES_2026-07-25.md`.
...
### تحديث إضافي 2026-07-25 — رسائل فشل رابط الإيميل
- تم تعديل شاشات التسجيل بالإيميل لتعرض سبب الفشل الحقيقي من السيرفر، بدل الرسالة العامة "فشل إرسال رابط الإيميل"، وخاصة حالة البريد المسجل مسبقاً.

### تحديث إضافي 2026-07-25 — معالجة حسابات Magic Link الناقصة
- تم تعديل `user-account` بحيث لا تمنع صفوف الإيميل الناقصة في `public.users` إعادة إرسال الرابط السحري، ويتم أرشفة الصف الناقص عند الحاجة قبل إنشاء حساب مطابق لجلسة Auth الجديدة.
