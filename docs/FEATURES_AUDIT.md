...
| ✅ إصلاح RLS policy على جدول offers (تغيير INSERT policy إلى `auth.uid() = usr_id`) — حل مشكلة "violates row-level security policy" | 2026-06-09 | `supabase/setup.sql` + `supabase/FUNCTIONS_REFERENCE.md` + `add_offer_screen.dart` + `offer_provider.dart` - إصلاح السياسة + توثيق + إزالة كل الـ prints المؤقتة (مطابقة لـ DEVELOPMENT_GUIDELINES) |
...
## 🆕 تحديث 2026-06-10 — حماية المسارات حسب الدور

تمت إضافة حماية مركزية في `GoRouter.redirect` لمنع فتح مسارات حساسة مباشرة بالرابط:

- `/admin/*` يحتاج مستخدماً مسجلاً بدور إداري (`role >= 2`).
- `/broker/*` يحتاج وسيطاً أو مديراً.
- `/user/*` يحتاج تسجيل دخول.
- الصفحات العامة مثل `/home`, `/search`, `/offer/:id`, `/login`, `/otp`, `/check-email`, `/splash` بقيت متاحة حسب المنطق الحالي.

الموقع:

- `lib/core/router/app_router.dart`

ملاحظة: هذه حماية دورية أولية متوافقة مع بنية المشروع الحالية. الصلاحيات الديناميكية التفصيلية موثقة للتنفيذ اللاحق في:

- `docs/INTERNAL_MANAGEMENT_ADAPTATION_PLAN.md`

---

## 🆕 تحديث 2026-06-10 — مركز عمليات المكتب

تمت إضافة شاشة تشغيلية مركزة داخل الإدارة باسم "مركز عمليات المكتب" لتجميع الأعمال اليومية دون تغيير نموذج البيانات الحالي:

- عروض بانتظار المراجعة.
- مدفوعات بانتظار الموافقة.
- تبليغات مفتوحة.
- طلبات توثيق.
- اختصارات إلى المواعيد، الصفقات، المستخدمين، والتحليلات.
- ملخص سريع للمستخدمين، العروض، المواعيد، والصفقات.
...
## 🆕 تحديث 2026-06-10 — Checklist الإدارة الداخلية

تمت إضافة قائمة اختبار يدوية للتأكد من:

- حماية مسارات الزائر/المستخدم/الوسيط/الإدارة.
- عمل شاشة إدارة الصلاحيات.
- ظهور كروت لوحة الإدارة حسب الصلاحيات.
- عمل مركز عمليات المكتب.
- عمل شاشة إدارة الوسائط والتصوير.
...
## 🆕 تحديث 2026-06-10 — Web manifest branding

تم تحديث `web/manifest.json` ليطابق هوية التطبيق بدلاً من قيم Flutter الافتراضية:

- الاسم: عقارات السويداء.
