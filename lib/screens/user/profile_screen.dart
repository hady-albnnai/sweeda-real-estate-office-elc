...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _login() async {
    final user = _logUser.text.trim();
    final pass = _logPass.text;
    ...
    try {
      ...
      if (!mounted) return;
      if (!ok) {
        _snack(auth.lastError ?? 'فشل تسجيل الدخول، تأكد من بياناتك');
      }
    } catch (e) {
      _snack('حدث خطأ في الاتصال بالسيرفر');
    } finally {
  ...
  Future<void> _forgot() async {
    ...
    try {
      ...
      if (mounted && ok) context.push('/otp');
      else if (mounted) _snack('فشل إرسال رسالة الاستعادة');
    } catch (e) {
      _snack('خطأ في إرسال SMS');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
  ...
  Future<void> _regSMS() async {
    ...
    if (ph.length != 10 || !ph.startsWith('09')) { _snack('أدخل رقم هاتف صحيح'); return; }
    setState(() => _busy = true);
    try {
      final ok = await context.read<AuthProvider>().sendSMSOTP(ph);
      if (mounted && ok) context.push('/otp');
      else if (mounted) _snack('فشل إرسال رمز التفعيل');
    } catch (e) {
      _snack('خطأ فني في الإرسال');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
  ...
  Future<void> _regMail() async {
    final mail = _regEmail.text.trim();
    if (mail.isEmpty || !mail.contains('@')) { _snack('أدخل بريداً صحيحاً'); return; }
    setState(() => _busy = true);
    try {
      final ok = await context.read<AuthProvider>().sendEmailMagicLink(mail);
      if (mounted && ok) context.push('/check-email');
      else if (mounted) _snack('فشل إرسال الرابط');
    } catch (e) {
      _snack('خطأ في إرسال الإيميل');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
    ...
    return Scaffold(
      ...
      body: CustomScrollView(slivers: [
        ...
        SliverPadding(padding: const EdgeInsets.all(20), sliver: SliverList(delegate: SliverChildListDelegate([
          ...
          const SizedBox(height: 20), _buildMenuSection(auth.userModel!), const SizedBox(height: 20), _buildLogoutButton(auth), const SizedBox(height: 40),
        ]))),
      ]),
      ...
      child: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(children: [
            ...
            // تم تصغير الخط هنا ليظهر على سطر واحد
