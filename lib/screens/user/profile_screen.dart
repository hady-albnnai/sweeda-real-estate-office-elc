...
/// ════════════════════════════════════════════════════════════════════
/// شاشة الحساب الشخصي (ProfileScreen):
///   - للزائر: واجهة مبسطة ومنظمة (دخول / تسجيل) مع شعار عملاق.
///   - للعضو: واجهة الملف الوظيفي أو الشخصي مع الإحصائيات.
/// ════════════════════════════════════════════════════════════════════
...
class _ProfileScreenState extends State<ProfileScreen> {
  // ─── متغيرات الحالة للإدارة ───
  Map<String, dynamic>? _staffStats;
  bool _loadingStats = false;

  // ─── متغيرات التحكم في واجهة الزائر ───
  int _activeBlock = 0; // 0: مغلق، 1: تسجيل جديد، 2: تسجيل دخول
  int _signupMethod = 0; // 0: مغلق، 1: هاتف، 2: إيميل
  ...
  Future<void> _handleLogin() async {
    ...
    if (!ok) _snack(auth.lastError ?? 'فشل تسجيل الدخول');
  }

  Future<void> _handleForgotPassword() async {
    final phone = _loginUserCtrl.text.trim();
    if (phone.length != 10 || !phone.startsWith('09')) {
      _snack('أدخل رقم هاتفك (09xxxxxxxx) في خانة اسم المستخدم أولاً');
      return;
    }
    setState(() => _isBusy = true);
  ...
  Future<void> _handleSignupPhone() async {
    ...
    if (phone.length != 10 || !phone.startsWith('09')) {
      _snack('يرجى إدخال رقم هاتف صحيح');
      return;
    }
    setState(() => _isBusy = true);
    final ok = await context.read<AuthProvider>().sendSMSOTP(phone);
    if (mounted) setState(() => _isBusy = false);
    if (ok) {
      _snack('تم إرسال كود التفعيل SMS');
      context.push('/otp');
    } else {
  ...
  Future<void> _handleSignupEmail() async {
    ...
    if (email.isEmpty || !email.contains('@')) {
      _snack('يرجى إدخال بريد إلكتروني صحيح');
      return;
    }
    setState(() => _isBusy = true);
          ...
          child: Column(
            children: [
                  ...
                  child: Stack(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          border: Border.all(color: AppTheme.primaryGold.withValues(alpha: 0.4), width: 4),
                        ),
                      ),
                      Container(
                        width: logoSize * 0.65,
              ...
              Text(
                ...
                style: GoogleFonts.cairo(color: AppTheme.primaryGold, fontSize: 26, fontWeight: FontWeight.w900),
              ),
              ...
              _buildMainBlock(
                ...
                child: Column(
                  children: [
                    ...
                    _buildInput(
                      _loginPassCtrl, 'كلمة المرور', Icons.lock_outline,
                      isPass: true,
                      obscure: _isPassObscure,
                      onToggle: () => setState(() => _isPassObscure = !_isPassObscure),
                    ),
                    const SizedBox(height: 20),
                    ...
                    TextButton(
                      ...
                      child: const Text(
                        'هل نسيت كلمة المرور؟ استعادة بـ SMS',
                        style: TextStyle(color: AppTheme.primaryGold, decoration: TextDecoration.underline, fontSize: 13),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildMainBlock({required int id, required String title, required IconData icon, required bool isGold, required Widget child}) {
  ...
  Widget _buildInnerOption({required int id, required String title, required IconData icon, required Widget child}) {
    final isSelected = _signupMethod == id;
    return Container(
      decoration: BoxDecoration(
      ...
      child: Column(
        children: [
          ListTile(
            onTap: () => setState(() => _signupMethod = isSelected ? 0 : id),
            leading: Icon(icon, color: Colors.black87),
            title: Text(title, style: const TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 14)),
            trailing: Icon(isSelected ? Icons.radio_button_checked : Icons.radio_button_off, color: Colors.black87),
          ),
  ...
  Widget _buildInput(TextEditingController c, String h, IconData i, {bool isPass = false, bool? obscure, VoidCallback? onToggle, bool dark = false}) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: h,
        prefixIcon: Icon(i, color: dark ? Colors.black45 : AppTheme.primaryGold),
  ...
  // ═══════════════════════════════════════════════════════════════
  //  واجهة العضو المسجل (العادية + الإدارة)
  // ═══════════════════════════════════════════════════════════════
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [AppTheme.primaryGold.withValues(alpha: 0.2), AppTheme.deepBlack],
        ),
      ),
      child: Column(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              gradient: const LinearGradient(colors: [AppTheme.primaryGold, AppTheme.lightGold]),
              boxShadow: [BoxShadow(color: AppTheme.primaryGold.withValues(alpha: 0.25), blurRadius: 20, spreadRadius: 2)],
            ),
            child: Container(
              margin: const EdgeInsets.all(3),
              decoration: const BoxDecoration(shape: BoxShape.circle, color: AppTheme.surfaceBlack),
              child: Center(child: Text(user.nm.isNotEmpty ? user.nm[0] : '؟', style: const TextStyle(color: AppTheme.primaryGold, fontSize: 36, fontWeight: FontWeight.bold))),
            ),
          ),
          const SizedBox(height: 12),
  ...
  Widget _buildUserStats(UserModel u) => Row(children: [
    Expanded(child: _buildStatTile(icon: Icons.star_rounded, value: '${u.pt}', label: 'النقاط', color: const Color(0xFFFFD700))),
    const SizedBox(width: 12),
    Expanded(child: _buildStatTile(icon: Icons.local_fire_department_rounded, value: '${u.strk}', label: 'أيام متتالية', color: const Color(0xFFFF6B35))),
  ]);
  ...
  Widget _buildStaffStats(UserModel user) {
    if (_loadingStats) return const Center(child: Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator(color: AppTheme.primaryGold)));
    if (_staffStats == null) return const SizedBox.shrink();
    List<_StaffStatItem> items = [];
    if (user.isPhotographer) {
      items = [_StaffStatItem(Icons.check_circle_outline, 'مهام مكتملة', _staffStats!['completed_tasks'] ?? 0, Colors.green)];
    } else if (user.isSupervisor) {
      items = [_StaffStatItem(Icons.check_circle_outline, 'زيارات منفذة', _staffStats!['completed_visits'] ?? 0, Colors.green)];
    } else if (user.isEmployee) {
      items = [_StaffStatItem(Icons.rate_review_outlined, 'عروض مراجَعة', _staffStats!['reviewed_offers'] ?? 0, Colors.blue)];
    } else if (user.isSenior || user.isManager) {
      items = [
        _StaffStatItem(Icons.handshake_outlined, 'صفقات', _staffStats!['total_deals'] ?? 0, Colors.green),
        _StaffStatItem(Icons.payments_outlined, 'مدفوعات', _staffStats!['approved_payments'] ?? 0, Colors.blue),
        _StaffStatItem(Icons.verified_user_outlined, 'موثقون', _staffStats!['verified_users'] ?? 0, Colors.teal),
      ];
    }
  ...
  Widget _buildMenuSection(UserModel u) => Column(children: [
    _buildMenuItem(i: Icons.person_outline, t: u.isAdmin ? 'بياناتي الوظيفية' : 'معلومات الحساب', s: u.isAdmin ? 'بيانات التعيين والتحقق الوظيفي ✅' : 'معلوماتك الشخصية والتوثيق', o: () => context.push('/user/account-info')),
    _buildMenuItem(i: Icons.star_outline, t: 'تقييماتي المستلمة', s: 'شاهد تقييمات العملاء لك', o: () => context.push('/user/my-ratings')),
    if (u.isAdmin) _buildMenuItem(i: Icons.dashboard_outlined, t: 'لوحة التحكم الإدارية', s: 'الانتقال لواجهة العمليات', o: () {
      if (u.isPhotographer) context.go('/photographer/tasks');
      else if (u.isSupervisor) context.go('/executor/tasks');
      else if (u.isEmployee) context.go('/employee/dashboard');
